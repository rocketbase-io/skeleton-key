...
function npmTaskAfterBuild(task, ...args) {
  ...
  process.on("beforeExit", () => proc && proc.kill(undefined, { forceKillAfterTimeout: 10 }));

  async function restartCommand() {
    if (proc !== undefined && !proc.killed) proc.kill(undefined, { forceKillAfterTimeout: 10 });
    proc = spawn("npm", ["run", task, ...args], {
